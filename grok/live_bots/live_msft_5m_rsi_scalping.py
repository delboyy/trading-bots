#!/usr/bin/env python3
"""
MSFT 5m RSI Scalping Bot - WINNER STRATEGY
Return: 4.00%, Win Rate: 53.7%, Trades: 54, Max DD: 4.50%

RSI-based scalping strategy optimized for MSFT stability.
Uses RSI extremes with volume confirmation and Bollinger Band filtering.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/msft_rsi_scalping.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('MSFT_RSI_SCALPING')

class MSFTRSIScalpingBot:
    """
    RSI-Based Scalping Bot for MSFT
    Optimized for 5-minute timeframe with oscillator signals
    """

    def __init__(self):
        self.tracker = StatusTracker()
        self.bot_id = "msft_5m_rsi"
        ...
        logger.info("🚀 MSFT RSI Scalping Bot initialized")
        logger.info(f"Strategy: {self.strategy_type}")
        logger.info(f"Symbol: {self.symbol}, Timeframe: 5m")
        logger.info(f"RSI Parameters: Period={self.rsi_period}, Oversold={self.rsi_oversold}, Overbought={self.rsi_overbought}")
        logger.info(f"Expected Performance: 4.00% return, 53.7% win rate")
    ...
    def generate_signal(self, df: pd.DataFrame) -> int:
        """Generate trading signal using RSI scalping logic"""
        if len(df) < 50:
            return 0

        current_time = df.index[-1]

        # Avoid overtrading - minimum 5 minutes between signals
        if self.last_signal_time and (current_time - self.last_signal_time).total_seconds() < 300:
            return 0
        ...
        # RSI-based signals with Bollinger Band filtering
        signal = 0

        # LONG SIGNAL: RSI oversold + near lower BB
        if (current_rsi < self.rsi_oversold and
            current_close < bb_lower * 1.01):  # Within 1% of lower BB
            signal = 1

        # SHORT SIGNAL: RSI overbought + near upper BB
        ...
        if signal != 0:
            ...
            logger.info(f"RSI Signal generated: {direction} at {current_time}")
    ...
    def run(self):
        """Main trading loop"""
        logger.info("Starting MSFT RSI Scalping Bot...")
