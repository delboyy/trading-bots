#!/usr/bin/env python3
"""
LIVE TRADING BOT: SLV 4h Mean Reversion
Strategy: Z-score mean reversion on Silver ETF
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/slv_4h_mean_reversion.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('SLV_4H_MEAN_REVERSION')

# Create logs directory
os.makedirs('logs', exist_ok=True)

class SLVMeanReversionBot:
    """Live trading bot for SLV 4h Mean Reversion strategy"""
    ...
    def __init__(self):
        ...
        # Risk management - BACKTESTED DRAWDOWN: 9.3% | WIN RATE: 91.7%
        # SAFE ACCOUNT SIZE: $5,000+ | RECOMMENDED POSITION SIZE: 3-5% | LOW RISK
        ...
        logger.info(f"Initialized SLV 4h Mean Reversion Bot - Window({self.window}), Z-threshold({self.z_thresh})")
    ...
    def get_historical_data(self, hours: int = 192) -> pd.DataFrame:
        """Get historical 4h data for SLV (192 hours = 8 days of 4h bars)"""
        try:
    ...
    def check_exit_condition(self, df: pd.DataFrame) -> bool:
        ...
        # Exit when z-score crosses back to neutral (close to zero)
        if abs(current_z) < 0.1:  # Close enough to zero
            return True
    ...
    def run_strategy(self):
        ...
        try:
            ...
            if current_position:
                ...
                df = self.get_historical_data(240)  # Extra data for calculations
            ...
            df = self.get_historical_data(240)  # Get enough data for rolling calculations
    ...
    def run_live(self):
        ...
        logger.info("Starting SLV 4h Mean Reversion live trading bot...")

        def job():
            self.run_strategy()

        # Schedule to run every 15 minutes (4h timeframe, so check for new bars)
